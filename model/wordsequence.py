...
class WordSequence(nn.Module):
    ...
    def forward(self, word_inputs, feature_inputs, word_seq_lengths):
        ...
        if self.word_feature_extractor == "CNN":
            ...
            packed_words = pack_padded_sequence(
                word_represent, word_seq_lengths.cpu().numpy(), batch_first=True
            )
            lstm_out, _ = self.lstm(packed_words, None)
            lstm_out, _ = pad_packed_sequence(lstm_out, batch_first=False)
    ...
    def sentence_representation(self, word_inputs, feature_inputs, word_seq_lengths):
        """
            input:
                word_inputs: (batch_size, sent_len)
        ...
        if self.word_feature_extractor == "CNN":
            ...
            for idx in range(self.cnn_layer):
                ...
                if batch_size > 1:
                    cnn_feature = self.cnn_batchnorm_list[idx](cnn_feature)
            feature_out = F.max_pool1d(cnn_feature, cnn_feature.size(2)).view(
                batch_size, -1
            )
        else:
            packed_words = pack_padded_sequence(
            ...
            # lstm_out (seq_len, seq_len, hidden_size)
            _, hidden = self.lstm(packed_words, None)
