...
def predict_check(
    pred_variable, gold_variable, mask_variable, sentence_classification=False
):
    """
        input:
...
def lr_decay(optimizer, epoch, decay_rate, init_lr):
    ...
    logger.info(f" Learning rate is set as:{lr}")
...
def evaluate(data, model, name, nbest=None):
    if name == "train":
        ...
        logger.info(f"Error: wrong evaluate name {name}")
        exit(1)
    nbest_pred_results = []
    pred_scores = []
    pred_results = []
...
def batchify_with_label(
    input_batch_list, gpu, if_train=True, sentence_classification=False
):
    if sentence_classification:
        return batchify_sentence_classification_with_label(
            input_batch_list, gpu, if_train
        )
    else:
...
def train(data):
    ...
    for idx in range(data.HP_iteration):
        ...
        logger.info(f"Shuffle: first input word list: {data.train_ids[0][0]}")
        ...
        for batch_id in range(total_batch):
            ...
            loss, tag_seq = model.calculate_loss(
                batch_word, batch_features, batch_wordlen, batch_label, mask
            )
            right, whole = predict_check(
                tag_seq, batch_label, mask, data.sentence_classification
            )
            right_token += right
            whole_token += whole
            ...
            if end % 500 == 0:
                ...
                if sample_loss > 1e8 or str(sample_loss) == "nan":
                    logger.info(
                        "ERROR: LOSS EXPLOSION (>1e8) ! PLEASE SET PROPER PARAMETERS "
                        "AND STRUCTURE! EXIT...."
                    )
                    exit(1)
                sys.stdout.flush()
        ...
        logger.info(f"totalloss: {total_loss}")
        if total_loss > 1e8 or str(total_loss) == "nan":
            logger.info(
                "ERROR: LOSS EXPLOSION (>1e8) ! PLEASE SET PROPER PARAMETERS AND "
                "STRUCTURE! EXIT...."
            )
            exit(1)
        # continue
        ...
        if data.seg:
            ...
            current_score = acc
            logger.info(
                "Dev: time: %.2fs speed: %.2fst/s; acc: %.4f" % (dev_cost, speed, acc)
            )
        ...
        if current_score > best_dev:
            ...
            logger.info(f"Save current best model in file: {model_name}")
        ...
        if data.seg:
            ...
            logger.info(
                "Test: time: %.2fs, speed: %.2fst/s; acc: %.4f"
                % (test_cost, speed, acc)
            )
        gc.collect()
...
def load_model_decode(data, name):
    logger.info(f"Load Model from file: {data.model_dir}")
    if data.sentence_classification:
        model = SentClassifier(data)
    else:
        model = SeqLabel(data)
    ...
    if data.seg:
        ...
        logger.info(
            "%s: time:%.2fs, speed:%.2fst/s; acc: %.4f" % (name, time_cost, speed, acc)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config", help="Configuration File", default="demo.train.config"
    )
    ...
    if args.config == "None":
        ...
        logger.info(f"Save dset directory: {data.dset_dir}")
        ...
        if args.seg.lower() == "true":
            ...
            data.seg = False
        logger.info(f"Seed num: {seed_num}")
    else:
